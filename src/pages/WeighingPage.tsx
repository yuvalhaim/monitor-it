...
interface WeighingPageProps {
  ...
  // controlled from App (sidebar selection)
...
export function WeighingPage({ token, userProfile, isDarkMode = true }: WeighingPageProps) {
  ...
  // unix-timestamp X axis (same pattern as energy HistoryChart)
  ...
  // ─── Theme — same palette as Energy page (index.css CSS variables) ─────────
  const C = isDarkMode ? {
    page:    "hsl(222, 47%, 11%)",          // --background dark
    card:    "hsl(217, 32%, 17%)",          // --card dark
    inner:   "hsl(222, 47%, 9%)",           // deeper dark
    border:  "hsl(215, 19%, 34%)",          // --border dark
    text:    "hsl(210, 40%, 98%)",          // --foreground dark (near white)
    muted:   "hsl(215, 20%, 60%)",          // medium grey
    sub:     "hsl(215, 20%, 75%)",          // lighter grey
  } : {
    page:    "#b8d4e8",                     // --background light (steel blue)
    card:    "#d4e8f6",                     // --card light
    inner:   "#c2daea",                     // --sidebar-bg light
    border:  "rgba(0,120,180,0.26)",        // --border light
    text:    "hsl(222, 47%, 11%)",          // --foreground light (near black)
    muted:   "hsl(213, 28%, 38%)",          // --muted light
    sub:     "hsl(222, 47%, 20%)",          // darker variant
  };
    ...
    <div dir="rtl" style={S.page}>
      ...
      <div className="weighing-hdr" style={{ display: "flex", alignItems: "center", marginBottom: 16, gap: 10 }}>
        <div className="weighing-hdr-left" style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <h1 style={{ fontFamily: "DM Sans, sans-serif", fontSize: 18, fontWeight: 700, margin: 0, color: C.text }}>
            {selectedDevice?.site_name ?? userProfile?.site_name ?? "מערכת שקילה"}
          </h1>
        </div>
            ...
            {devices.map(d => (
              <button key={d.id_user} onClick={() => setSelectedId(d.id_user)}
                style={{
                  ...
                  background: selectedId === d.id_user ? "hsl(198,93%,59%,0.15)" : "transparent",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20, opacity: loading ? 0.45 : 1, transition: "opacity 0.2s", position: "relative", pointerEvents: loading ? "none" : "auto" }}>
          ...
          <div className="widget-card" style={{ ...S.card, padding: "28px 32px", overflow: "hidden" }}>
            <div className="widget-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32, alignItems: "center" }}>
              ...
              {/* Widget — right column (col-1 in RTL = visual right) */}
      ...
      <style>{`
        ...
        @media(max-width:600px){
          ...
          .weighing-top-row      { flex-direction:column!important; }
          .weighing-top-row > *  { width:100%!important; min-width:0!important; box-sizing:border-box; }
          .weighing-ts-card { padding:10px 14px!important; flex-direction:row!important; align-items:center!important; flex-wrap:wrap!important; gap:4px 14px!important; justify-content:flex-start!important; }
          .weighing-ts-card > div:nth-child(1) { margin-bottom:0!important; }
          .weighing-ts-card > div:nth-child(1) span { font-size:14px!important; }
          .weighing-ts-card > div:nth-child(2) { font-size:24px!important; }
          .weighing-ts-card > div:nth-child(3) { font-size:24px!important; margin-top:0!important; }
          .weighing-ts-card > div:nth-child(4) { margin-top:0!important; }
